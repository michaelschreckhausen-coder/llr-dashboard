...
- **activities.type CHECK-Constraint Prod ↔ Staging-Drift** (entdeckt 2026-05-13 beim Prod-LinkedIn-Seed): Prod-`activities_type_check` erlaubt `['call','email','linkedin_message','linkedin_visit','meeting','note','task','whatsapp','sms','other']` — `linkedin_connection` ist **nicht** im Set. Staging hat aktuell **gar keinen** CHECK. Vernetzungen.jsx schreibt aber `type:'linkedin_connection'` beim Vernetzungsanfrage-Senden (Line ~94) → würde auf Prod silent fail werfen. Demo-Seed canonical-isiert jetzt auf `linkedin_message` mit Subject-Variation (`'LinkedIn-Vernetzungsanfrage gesendet'` vs `'LinkedIn-Nachricht (gesendet)'`). Eigener Backlog-PR: entweder CHECK-Constraint um `linkedin_connection` erweitern (+ Staging mit gleichem Constraint angleichen) oder Frontend auf `linkedin_message` umstellen.
