...
Deno.serve(async (req) => {
  ...
  const _rawPerDay = Number((camp.caps as any)?.[step0.action]?.per_day ?? (camp.caps as any)?.per_day) || DEFAULT_PER_DAY;
  const _maxPerDay = step0.action === "invite" ? 40 : 100;  // Schutz: LinkedIn ~100 Anfragen/Woche; Nachrichten 30-100/Tag
  const perDay: number = Math.min(_maxPerDay, Math.max(1, _rawPerDay)); // Guard: 0/NaN -> kein Div-by-0-Crash
