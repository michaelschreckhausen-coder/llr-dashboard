...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let loginList: any[] = [];
    {
      // Kandidaten: Verbindungen der Team-Marken UND die EIGENE Verbindung des Aufrufers.
      // Per-Lizenz: der User darf seine eigene LinkedIn-Verbindung fuer seine Company-Brand
      // nutzen, auch wenn seine Personal-Marke in einem ANDEREN Team liegt (Multi-Team-User).
      const _orFilter = brandIds.size > 0
        ? `brand_voice_id.in.(${Array.from(brandIds).join(",")}),user_id.eq.${auth.userId}`
