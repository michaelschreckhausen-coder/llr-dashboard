-- 20260610120000_whitelabel_phase1_admin_update_plan_feature_flag.sql
--
-- Whitelabel Phase 1 — admin_update_plan um feature_whitelabel (boolean) erweitern.
-- Baut auf 20260601125000_credits_phase1_admin_update_plan_extended_fields.sql auf;
-- vollstaendige Neudefinition (DROP+CREATE) mit einem zusaetzlichen Boolean-Branch
-- (analog is_team_plan/is_active). Restliche Felder unveraendert.
--
-- Setzt plans.feature_whitelabel voraus (20260610100000). Idempotent.
...
BEGIN
  ...
  CASE p_field_name
    ...
    -- ── Whitelabel-Gate ──────────────────────────────────────────────────────
    WHEN 'feature_whitelabel' THEN
      IF jsonb_typeof(p_new_value) != 'boolean' THEN
        RAISE EXCEPTION 'feature_whitelabel must be jsonb-boolean' USING ERRCODE = '22023';
      END IF;
      SELECT to_jsonb(feature_whitelabel) INTO v_before FROM public.plans WHERE id = p_plan_id;
      UPDATE public.plans SET feature_whitelabel = (p_new_value)::boolean, updated_at = now() WHERE id = p_plan_id;
