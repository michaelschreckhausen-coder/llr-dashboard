...
// Liest LinkedIn-Profildaten aus dem DOM und sendet sie ans Dashboard
...
(function() {
  ...
  function extractProfile() {
    ...
    // Name
    p.li_name = getAny(['h1.text-heading-xlarge','h1[class*="artdeco"] span','h1']);
    ...
    // Unternehmen — aus aktuellem Job oder Top-Card
    const exp = document.querySelector('#experience');
    if (exp) {
      const sec = exp.closest('section') || exp.parentElement;
      p.li_company = sec ? (get('.hoverable-link-text .t-bold span[aria-hidden]',sec) || get('.t-bold span[aria-hidden]',sec)) : '';
    }
    if (!p.li_company) {
  ...
  chrome.runtime.onMessage.addListener(function(msg, sender, sendResponse) {
    if (msg.type === 'GET_PROFILE') {
      try {
        ...
        t.style.cssText = 'position:fixed;bottom:24px;right:24px;background:#0A66C2;color:#fff;padding:12px 20px;border-radius:10px;font-family:-apple-system,sans-serif;font-size:14px;font-weight:700;z-index:999999;box-shadow:0 4px 20px rgba(0,0,0,.2)';
        t.textContent = '\uD83C\uDFAF ' + (profile.li_name||'Profil') + ' importiert!';
