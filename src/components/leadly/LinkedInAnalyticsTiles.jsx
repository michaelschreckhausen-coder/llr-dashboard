...
// auf EINEN Blick — gruppiert (Handlungsbedarf · Netzwerk · Content · LinkedIn→CRM),
// dichtes responsives Raster (mehrspaltig). Jede Kachel = KPI + Deep-Link.
...
export default function LinkedInAnalyticsTiles() {
  ...
  // Vier thematische Blöcke — alles auf einen Blick, je Block eine eigene mehrspaltige Reihe.
  ...
  const sections = [
    ...
    { label: 'LinkedIn → CRM', color: GREEN, tiles: [
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6, marginBottom: 4 }}>
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10.5, fontWeight: 600, color: 'var(--text-muted, #6B7280)' }}>
          <span style={{ width: 7, height: 7, borderRadius: 999, background: '#0F766E' }} />Team-weit · alle Marken
        </span>
      </div>

      {activeTeamId && (
        <div key={tick} style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 8 }}>
          {sections.map((s) => (
            <div key={s.label}>
