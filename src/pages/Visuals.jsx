...
// ─── Lade-Animation während der Bild-Generierung ─────────────────────────
// 30-90 Sekunden Wartezeit sind lang. Statt nur Spinner: rotierende Status-
// Texte + optionaler Mini-Snake (LocalStorage-High-Score). Tabwechsel via
// Button im Header.
...
function GenerationLoading({ premium }) {
  ...
  const P = 'var(--wl-primary, rgb(49,90,231))'
  const expectedMax = premium ? 90 : 30
  ...
  return (
    <section style={{
      ...
      padding: 20,
      borderRadius: 16,
      background: 'linear-gradient(135deg, rgba(49,90,231,0.04) 0%, rgba(139,92,246,0.06) 100%)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, gap: 10, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{
            ...
            animation: 'visualsSpin 0.9s linear infinite',
          }} />
          <div>
      ...
      <div style={{ position: 'relative', height: 6, borderRadius: 4, background: 'rgba(49,90,231,0.12)', overflow: 'hidden', marginBottom: 14 }}>
        ...
        <div style={{
          ...
          animation: 'visualsShimmer 1.8s linear infinite',
        }} />
      </div>
      ...
      <style>{`
        @keyframes visualsSpin { to { transform: rotate(360deg); } }
        @keyframes visualsShimmer { 0% { transform: translateX(-60px); } 100% { transform: translateX(800px); } }
        @keyframes visualsBrushBob {
...
function ArtScene({ status }) {
  // 24 floating coloured dots with deterministic positions/colors
  const dots = []
        ...
        {dots.map((d, i) => (
          <div key={i}
            style={{
              ...
              animation: 'visualsDotFloat 3.2s ease-in-out infinite',
              animationDelay: d.delay + 's',
              '--dx': d.dx + 'px',
              '--dy': d.dy + 'px',
            }}/>
        ))}
        {/* Brush emoji bobbing in the middle */}
        <div style={{
          position: 'absolute', left: '50%', top: '50%',
          transform: 'translate(-50%, -50%)',
          fontSize: 44,
          animation: 'visualsBrushBob 2.4s ease-in-out infinite',
        }}>🎨</div>
      </div>
      <div key={status} style={{
        fontSize: 13, fontWeight: 600, color: 'var(--text-primary)',
        textAlign: 'center', minHeight: 22,
        animation: 'visualsStatusFade 0.5s ease-out',
      }}>{status}</div>
    </div>
  )
}
...
function SnakeGame({ primaryColor }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: '8px 0' }}>
      ...
      {/* On-Screen Dpad (Mobile + Maus-Klick-Friendly) */}
