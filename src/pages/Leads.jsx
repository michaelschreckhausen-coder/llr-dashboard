...
const ListIcon  = () => <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>
const SearchIcon= () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
const FilterIcon= () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="4" y1="6" x2="20" y2="6"/><line x1="8" y1="12" x2="16" y2="12"/><line x1="11" y1="18" x2="13" y2="18"/></svg>
const GridIcon  = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>
const SortIcon  = () => <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="16" y2="12"/><line x1="8" y1="18" x2="11" y2="18"/></svg>
const LinkIcon  = () => <svg width="11" height="11" viewBox="0 0 24 24" fill="white"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg>
const CloseIcon = () => <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
...
function IconBtn({ onClick, title, danger, children }) {
  ...
  return (
    <button onClick={e => { e.stopPropagation(); onClick(e) }} title={title}
      style={{ width:30, height:30, borderRadius:8, border:'1px solid '+(hov?(danger?'#FCA5A5':'#BFDBFE'):'#E2E8F0'), background:hov?(danger?'#FEF2F2':'#EFF6FF'):'transparent', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:hov?(danger?'#EF4444':'#0A66C2'):'#64748B', transition:'all 0.15s' }}
...
function SearchBar({ leads, onResults }) {
  ...
  const inpStyle   = { padding:'8px 12px', border:'1.5px solid #E2E8F0', borderRadius:8, fontSize:13, outline:'none', fontFamily:'Inter,sans-serif', background:'#fff', color:'#0F172A', transition:'border 0.15s' }
    ...
    <div style={{ background:'#fff', borderBottom:'1px solid #E2E8F0', padding:'12px 20px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        ...
        <div style={{ position:'relative', flex:1, maxWidth:440 }}>
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Name, Unternehmen oder Stichwort…"
            style={{ ...inpStyle, width:'100%', paddingLeft:34, paddingRight:query?32:12 }}
            onFocus={e => e.target.style.borderColor='#0A66C2'} onBlur={e => e.target.style.borderColor='#E2E8F0'}/>
          {query && <button onClick={() => setQuery('')} style={{ position:'absolute', right:9, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:'#94A3B8', display:'flex', padding:2 }}><CloseIcon/></button>}
        </div>
        {/* Filter button */}
        ...
        {hasFilters && <button onClick={clearAll} style={{ fontSize:12, color:'#EF4444', background:'none', border:'none', cursor:'pointer', fontWeight:600, whiteSpace:'nowrap' }}>Zurücksetzen</button>}
      </div>
      ...
      {showFilters && (
        <div style={{ display:'flex', gap:14, flexWrap:'wrap', marginTop:12, paddingTop:12, borderTop:'1px solid #F1F5F9' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
            ...
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ ...inpStyle, cursor:'pointer', minWidth:150 }}
              onFocus={e => e.target.style.borderColor='#0A66C2'} onBlur={e => e.target.style.borderColor='#E2E8F0'}>
      ...
      {chips.length > 0 && (
        <div style={{ display:'flex', gap:7, flexWrap:'wrap', alignItems:'center', marginTop:10 }}>
          <span style={{ fontSize:11, color:'#94A3B8', fontWeight:600 }}>Filter aktiv:</span>
          {chips.map((chip, i) => (
            <span key={i} style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'3px 10px', borderRadius:999, background:'#EFF6FF', color:'#1D4ED8', fontSize:12, fontWeight:600, border:'1px solid #BFDBFE' }}>
              {chip.label}
              <button onClick={chip.clear} style={{ background:'none', border:'none', cursor:'pointer', color:'#1D4ED8', display:'flex', padding:0 }}><CloseIcon/></button>
            </span>
          ))}
        </div>
      )}
...
export default function Leads({ session }) {
  ...
  return (
    <div style={{ display:'flex', height:'calc(100vh - 56px)', overflow:'hidden', margin:'-28px -32px', background:'#F1F5F9' }}>
      ...
      {/* ── LEFT SIDEBAR (Lists) ── */}
      <aside style={{ width:228, flexShrink:0, background:'#fff', borderRight:'1px solid #E2E8F0', display:'flex', flexDirection:'column', overflow:'hidden' }}>
        {/* Header */}
        <div style={{ padding:'14px 16px', borderBottom:'1px solid #E2E8F0', display:'flex', justifyContent:'space-between', alignItems:'center', minHeight:48 }}>
          <span style={{ fontSize:10, fontWeight:700, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.1em' }}>Listen</span>
          <button onClick={openNewList} title="Neue Liste"
            style={{ width:24, height:24, borderRadius:6, background:'#0A66C2', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', transition:'background 0.15s', boxShadow:'0 1px 3px rgba(10,102,194,0.3)' }}
            onMouseOver={e => e.currentTarget.style.background='#0958A8'}
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'6px 8px' }}>
          ...
          <div onClick={() => setListFilter('all')}
            style={{ padding:'8px 12px', cursor:'pointer', borderRadius:8, marginBottom:2, display:'flex', alignItems:'center', gap:9, fontSize:13, fontWeight:listFilter==='all'?700:500, color:listFilter==='all'?'#0A66C2':'#475569', background:listFilter==='all'?'#EFF6FF':'transparent', borderLeft:'3px solid '+(listFilter==='all'?'#0A66C2':'transparent'), transition:'all 0.12s' }}>
            <span style={{ color:listFilter==='all'?'#0A66C2':'#94A3B8', display:'flex', flexShrink:0 }}><GridIcon/></span>
          ...
          {lists.map(list => (
            <div key={list.id} onClick={() => setListFilter(list.id)}
              style={{ padding:'8px 12px', cursor:'pointer', borderRadius:8, marginBottom:2, display:'flex', alignItems:'center', gap:9, fontSize:13, fontWeight:listFilter===list.id?700:500, color:listFilter===list.id?list.color:'#475569', background:listFilter===list.id?list.color+'16':'transparent', borderLeft:'3px solid '+(listFilter===list.id?list.color:'transparent'), transition:'all 0.12s' }}>
              ...
              <div style={{ display:'flex', gap:2, alignItems:'center' }}>
                ...
                <button onClick={e => { e.stopPropagation(); openEditList(list) }}
                  style={{ background:'none', border:'none', cursor:'pointer', color:'#CBD5E1', padding:3, borderRadius:5, display:'flex', transition:'color 0.12s' }}
                  onMouseOver={e => e.currentTarget.style.color='#64748B'}
                  onMouseOut={e => e.currentTarget.style.color='#CBD5E1'}><EditIcon/></button>
                <button onClick={e => { e.stopPropagation(); deleteList(list.id) }}
                  style={{ background:'none', border:'none', cursor:'pointer', color:'#CBD5E1', padding:3, borderRadius:5, display:'flex', transition:'color 0.12s' }}
                  onMouseOver={e => e.currentTarget.style.color='#EF4444'}
                  onMouseOut={e => e.currentTarget.style.color='#CBD5E1'}><TrashIcon/></button>
              </div>
            </div>
          ))}

          {lists.length === 0 && (
            <div style={{ padding:'16px 12px', fontSize:12, color:'#94A3B8', lineHeight:1.7, textAlign:'center' }}>
              <div style={{ fontSize:24, marginBottom:6 }}>📋</div>
              Noch keine Listen.<br/>
              <button onClick={openNewList} style={{ marginTop:6, fontSize:12, fontWeight:700, color:'#0A66C2', background:'none', border:'none', cursor:'pointer', padding:0 }}>+ Erstellen</button>
            </div>
          )}
        </div>
      </aside>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
        ...
        {/* Top bar */}
        <div style={{ background:'#fff', borderBottom:'1px solid #E2E8F0', padding:'0 20px', height:52, display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            {listFilter !== 'all' && activeListColor && <div style={{ width:10, height:10, borderRadius:'50%', background:activeListColor }}/>}
            <span style={{ fontSize:15, fontWeight:800, color:'#0F172A', letterSpacing:'-0.01em' }}>{activeListName}</span>
            <span style={{ fontSize:13, color:'#94A3B8', fontWeight:500 }}>({visibleLeads.length} {visibleLeads.length===1?'Lead':'Leads'})</span>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ display:'flex', alignItems:'center', gap:6 }}>
              <span style={{ display:'flex', color:'#94A3B8' }}><SortIcon/></span>
              <select value={sortBy} onChange={e => setSortBy(e.target.value)}
                style={{ fontSize:12, padding:'5px 10px', borderRadius:8, border:'1px solid #E2E8F0', background:'#F8FAFC', color:'#475569', cursor:'pointer', outline:'none', fontFamily:'Inter,sans-serif' }}>
            ...
            <button onClick={openAdd}
              style={{ display:'flex', alignItems:'center', gap:7, padding:'8px 16px', borderRadius:999, background:'#0A66C2', color:'#fff', border:'none', fontSize:13, fontWeight:700, cursor:'pointer', boxShadow:'0 1px 3px rgba(10,102,194,0.3)', transition:'all 0.18s' }}
        ...
        <div style={{ background:'#F8FAFC', borderBottom:'1px solid #E2E8F0', padding:'0 20px', height:36, display:'flex', alignItems:'center', flexShrink:0 }}>
          ...
          <div style={{ flex:1, fontSize:10, fontWeight:700, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.09em' }}>Name & Unternehmen</div>
          <div style={{ width:130, flexShrink:0, fontSize:10, fontWeight:700, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.09em' }}>Liste</div>
          <div style={{ width:145, flexShrink:0, fontSize:10, fontWeight:700, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.09em' }}>Status</div>
          <div style={{ width:90, flexShrink:0, fontSize:10, fontWeight:700, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.09em', textAlign:'right' }}>Datum</div>
          <div style={{ width:120, flexShrink:0, fontSize:10, fontWeight:700, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.09em', textAlign:'right' }}>Aktionen</div>
        </div>
        ...
        <div style={{ flex:1, overflowY:'auto', background:'#fff' }}>
          {loading ? (
            <div style={{ padding:48, textAlign:'center', color:'#94A3B8', fontSize:14 }}>
              <div style={{ fontSize:32, marginBottom:12 }}>⏳</div>Lade Leads…
            </div>
            ...
            <div style={{ padding:56, textAlign:'center' }}>
              <div style={{ width:56, height:56, borderRadius:'50%', background:'#F1F5F9', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 12px' }}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth="1.5" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              </div>
              <div style={{ fontSize:15, fontWeight:700, color:'#475569', marginBottom:6 }}>
                {leads.length === 0 ? 'Noch keine Leads' : 'Keine Ergebnisse'}
              </div>
              <div style={{ fontSize:13, color:'#94A3B8', maxWidth:280, margin:'0 auto 16px' }}>
                {leads.length === 0 ? 'Füge deinen ersten Lead hinzu und starte den Vertriebsprozess.' : 'Versuche andere Suchbegriffe oder Filter.'}
              </div>
              {leads.length === 0 && (
                <button onClick={openAdd} style={{ padding:'9px 22px', borderRadius:999, background:'#0A66C2', color:'#fff', border:'none', fontSize:13, fontWeight:700, cursor:'pointer', display:'inline-flex', alignItems:'center', gap:7 }}>
                  <PlusIcon/> Ersten Lead hinzufügen
                </button>
              )}
            </div>
            ...
            return (
              <div key={l.id}
                style={{ display:'flex', alignItems:'center', padding:'0 20px', minHeight:68, borderBottom:'1px solid #F1F5F9', background:isHov?'#F8FAFC':'#fff', transition:'background 0.12s', cursor:'default' }}
                onMouseEnter={() => setHoveredRow(l.id)} onMouseLeave={() => setHoveredRow(null)}>
                ...
                <div style={{ width:52, flexShrink:0 }}>
                  {l.avatar_url
                    ...
                    : <div style={{ width:40, height:40, borderRadius:'50%', background:'linear-gradient(135deg, #0A66C2, #3B82F6)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:15, fontWeight:800, color:'#fff', border:'2px solid #E2E8F0', boxShadow:'0 0 0 1px #E2E8F0' }}>
                ...
                {/* Lists */}
                <div style={{ width:130, flexShrink:0, display:'flex', flexWrap:'wrap', gap:4, alignItems:'center' }}>
                  {leadLists.map(list => (
                    <span key={list.id} style={{ padding:'2px 7px', borderRadius:999, fontSize:10, fontWeight:700, background:list.color+'16', color:list.color, border:'1px solid '+list.color+'30' }}>{list.name}</span>
                  ))}
                  <button onClick={e => { e.stopPropagation(); setAssignModal(l) }}
                    style={{ background:'none', border:'none', cursor:'pointer', color:'#CBD5E1', padding:3, borderRadius:5, display:'flex', transition:'color 0.12s' }}
                    title="Listen zuweisen" onMouseOver={e => e.currentTarget.style.color='#64748B'} onMouseOut={e => e.currentTarget.style.color='#CBD5E1'}>
                    <ListIcon/>
                  </button>
                </div>
                ...
                {/* Actions */}
                <div style={{ width:120, flexShrink:0, display:'flex', gap:5, justifyContent:'flex-end', alignItems:'center' }}>
                  {isHov && l.profile_url && (
                    <a href={l.profile_url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()}
                      style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 11px', borderRadius:999, background:'#0A66C2', color:'#fff', border:'none', fontSize:11, fontWeight:700, cursor:'pointer', textDecoration:'none', whiteSpace:'nowrap', boxShadow:'0 1px 3px rgba(10,102,194,0.3)' }}>
                      <LinkIcon/> Profil
                    </a>
                  )}
                  <IconBtn onClick={() => openEdit(l)} title="Bearbeiten"><EditIcon/></IconBtn>
                  <IconBtn onClick={() => deleteLead(l.id)} title="Löschen" danger><TrashIcon/></IconBtn>
                </div>
              </div>
            )
          })}
        ...
        <Modal title={modal==='add'?'Lead hinzufügen':'Lead bearbeiten'} onClose={closeModal}>
          <div style={{ padding:'18px 22px', display:'flex', flexDirection:'column', gap:14 }}>
            {[['name','Name *',true],['company','Unternehmen',false],['headline','Position / Headline',false],['profile_url','LinkedIn URL',false]].map(([key,label,req]) => (
              <div key={key}>
                ...
                <input value={form[key]||''} onChange={e => setForm(f => ({...f,[key]:e.target.value}))} style={inp}
                  placeholder={req?'Pflichtfeld':'Optional'}
                  onFocus={e => e.target.style.borderColor='#0A66C2'} onBlur={e => e.target.style.borderColor='#E2E8F0'}/>
              </div>
            ))}
            <div>
            ...
            <div>
              ...
              <textarea value={form.notes||''} onChange={e => setForm(f => ({...f,notes:e.target.value}))} rows={3}
                style={{ ...inp, resize:'vertical', lineHeight:1.5 }} placeholder="Persönliche Notizen…"
                onFocus={e => e.target.style.borderColor='#0A66C2'} onBlur={e => e.target.style.borderColor='#E2E8F0'}/>
            </div>
          </div>
          <div style={{ padding:'12px 22px 18px', display:'flex', justifyContent:'flex-end', gap:10, borderTop:'1px solid #F1F5F9' }}>
            ...
            <button onClick={save} disabled={saving||!form.name}
              style={{ padding:'8px 20px', borderRadius:999, border:'none', background:'#0A66C2', color:'#fff', fontSize:13, fontWeight:700, cursor:'pointer', opacity:!form.name?0.5:1, display:'flex', alignItems:'center', gap:7 }}>
              {saving ? '⏳' : <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg> Speichern</>}
            </button>
          </div>
        </Modal>
      )}
        ...
        <Modal title={listModal==='new'?'Neue Liste erstellen':'Liste bearbeiten'} onClose={closeListModal} width={380}>
          <div style={{ padding:'18px 22px', display:'flex', flexDirection:'column', gap:14 }}>
            <div>
              ...
              <input value={listForm.name||''} onChange={e => setListForm(f => ({...f,name:e.target.value}))} style={inp}
                placeholder="z.B. Potenzielle Kunden Q2"
                onFocus={e => e.target.style.borderColor='#0A66C2'} onBlur={e => e.target.style.borderColor='#E2E8F0'}/>
            </div>
            <div>
              <label style={lbl}>Beschreibung</label>
              <input value={listForm.description||''} onChange={e => setListForm(f => ({...f,description:e.target.value}))} style={inp}
              ...
              <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:6 }}>
                {LIST_COLORS.map(c => (
                  <button key={c} onClick={() => setListForm(f => ({...f,color:c}))}
                    style={{ width:28, height:28, borderRadius:'50%', background:c, border:listForm.color===c?'3px solid #0F172A':'3px solid transparent', cursor:'pointer', transition:'transform 0.12s, border 0.12s', boxShadow:listForm.color===c?'0 0 0 2px '+c+'44':'' }}
          ...
          <div style={{ padding:'12px 22px 18px', display:'flex', justifyContent:'flex-end', gap:10, borderTop:'1px solid #F1F5F9' }}>
            ...
            <button onClick={saveList} disabled={saving||!listForm.name}
              style={{ padding:'8px 20px', borderRadius:999, border:'none', background:'#0A66C2', color:'#fff', fontSize:13, fontWeight:700, cursor:'pointer', opacity:!listForm.name?0.5:1, display:'flex', alignItems:'center', gap:7 }}>
              {saving ? '⏳' : <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg> Speichern</>}
            </button>
          </div>
        </Modal>
      ...
      {assignModal && (
        <Modal title="Listen zuweisen" onClose={() => setAssignModal(null)} width={360}>
          <div style={{ padding:'14px 22px' }}>
            ...
            {lists.length === 0 ? (
              <div style={{ textAlign:'center', color:'#94A3B8', fontSize:13, padding:'16px 0' }}>
                Noch keine Listen.<br/>
                <button onClick={() => { setAssignModal(null); openNewList() }} style={{ marginTop:8, padding:'6px 16px', borderRadius:999, background:'#0A66C2', color:'#fff', border:'none', fontSize:12, fontWeight:700, cursor:'pointer', display:'inline-flex', alignItems:'center', gap:6 }}>
                  <PlusIcon/> Neue Liste
                </button>
              </div>
            ) : (
          ...
          <div style={{ padding:'10px 22px 18px', textAlign:'right', borderTop:'1px solid #F1F5F9' }}>
            <button onClick={() => { setAssignModal(null); loadAll() }} style={{ padding:'8px 20px', borderRadius:999, background:'#0A66C2', color:'#fff', border:'none', fontSize:13, fontWeight:700, cursor:'pointer' }}>Fertig</button>
          </div>
        </Modal>
      )}
    </div>
