...
const E0={name:'',industries:'',job_titles:'',company_sizes:'',locations:'',keywords:'',pain_points:'',is_default:false}
...
<label style={{fontSize:11,fontWeight:700,color:'#475569',textTransform:'uppercase',display:'block',marginBottom:5}}>{label}</label>
...
{tags.map((t,i)=>(<span key={i} style={{display:'inline-flex',alignItems:'center',gap:3,padding:'2px 8px',background:'#EFF6FF',color:'#0A66C2',borderRadius:999,fontSize:12,fontWeight:600}}>{t}<span onClick={()=>rm(i)} style={{cursor:'pointer',marginLeft:3}}>x</span></span>))}
<input value={v} onChange={e=>setV(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'||e.key===','){e.preventDefault();add()}}} placeholder={tags.length===0?placeholder:'+ add'} style={{border:'none',outline:'none',fontSize:12,minWidth:80,flex:1,background:'transparent'}}/>
</div></div>)}
...
const s={width:'100%',padding:'9px 12px',border:'1.5px solid #E2E8F0',borderRadius:9,fontSize:13,fontFamily:'inherit',boxSizing:'border-box',outline:'none'}
...
<h1 style={{fontSize:19,fontWeight:800,margin:0}}>{editing==='new'?'Neues ICP':'ICP bearbeiten'}</h1>
</div>
...
<input value={form.name} onChange={e=>sf('name')(e.target.value)} placeholder="z.B. DACH B2B Entscheider" style={s}/>
</div>
...
<textarea value={form.pain_points||''} onChange={e=>sf('pain_points')(e.target.value)} rows={2} style={{...s,resize:'vertical'}}/>
</div>
<label style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer',fontSize:13,marginBottom:20}}>
<input type="checkbox" checked={form.is_default||false} onChange={e=>sf('is_default')(e.target.checked)}/> Als Standard-ICP
</label>
</div>
...
<div><h1 style={{fontSize:22,fontWeight:800,margin:0}}>Zielgruppen (ICP)</h1><p style={{color:'#64748B',fontSize:13,margin:'4px 0 0'}}>Ideal Customer Profiles</p></div>
<button onClick={()=>{setForm(E0);setEditing('new')}} style={{padding:'9px 18px',borderRadius:9,background:'linear-gradient(135deg,#0A66C2,#8B5CF6)',color:'#fff',border:'none',fontSize:13,fontWeight:700,cursor:'pointer'}}>+ Neues ICP</button>
</div>
{icps.length===0?(
...
<button onClick={()=>{setForm(E0);setEditing('new')}} style={{padding:'10px 24px',borderRadius:18,background:'linear-gradient(135deg,#0A66C2,#8B5CF6)',color:'#fff',border:'none',fontSize:13,fontWeight:700,cursor:'pointer'}}>ICP erstellen</button>
</div>
):(
...
{!icp.is_default&&<button onClick={async()=>{await supabase.from('icp_profiles').update({is_default:false}).eq('user_id',session.user.id);await supabase.from('icp_profiles').update({is_default:true}).eq('id',icp.id);load()}} style={{padding:'6px 11px',borderRadius:7,border:'1px solid #E2E8F0',background:'#F8FAFC',fontSize:11,cursor:'pointer',color:'#475569'}}>Aktivieren</button>}
<button onClick={()=>{setForm({...icp,industries:a2s(icp.industries),job_titles:a2s(icp.job_titles),company_sizes:a2s(icp.company_sizes),locations:a2s(icp.locations),keywords:a2s(icp.keywords)});setEditing(icp)}} style={{padding:'6px 11px',borderRadius:7,border:'1px solid #E2E8F0',background:'#F8FAFC',fontSize:11,cursor:'pointer',color:'#475569'}}>Bearbeiten</button>
<button onClick={async()=>{if(window.confirm('Loeschen?')){await supabase.from('icp_profiles').delete().eq('id',icp.id);load()}}} style={{padding:'6px 11px',borderRadius:7,border:'1px solid #FCA5A5',background:'#FEF2F2',fontSize:11,cursor:'pointer',color:'#DC2626'}}>x</button>
</div></div></div>))}
</div>
)}
</div>)
