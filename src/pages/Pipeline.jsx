        ...
        <div style={{ padding:'20px 22px', display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <div>
            ...
            <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
              <input value={ico} onChange={e => setIco(e.target.value)} style={{ ...inp, width:64, fontSize:20, textAlign:'center' }} maxLength={4}/>
              {ICOS.map(em => (
                <button key={em} onClick={() => setIco(em)} style={{ background:ico===em?'#EFF6FF':'#F8FAFC', border:'1px solid '+(ico===em?'#BFDBFE':'#E2E8F0'), borderRadius:8, padding:'4px 8px', cursor:'pointer', fontSize:18 }}>{em}</button>
              ))}
            </div>
          </div>
          <div>
          ...
          <div style={{ background:'#F8FAFC', borderRadius:10, padding:'10px 14px', display:'flex', alignItems:'center', gap:10, border:'1px solid #E2E8F0' }}>
            ...
            <span style={{ marginLeft:'auto', fontSize:11, fontWeight:700, padding:'2px 10px', borderRadius:999, background:clr.bg, color:clr.color, border:'1px solid '+clr.border }}>0</span>
          </div>
        </div>
