...
export default function Pipeline({ session }) {
  ...
  function handleSaveCol(u){const n=cols.map(x=>x.id===u.id?{...x,...u}:x);setCols(n);saveCols(n);setEditCol(null)}
...
function EditColModal({col,onSave,onClose,onReset}){
  const C=[{color:'#475569',bg:'#F1F5F9',border:'#CBD5E1'},{color:'#1D4ED8',bg:'#EFF6FF',border:'#BFDBFE'},{color:'#6D28D9',bg:'#F5F3FF',border:'#DDD6FE'},{color:'#B45309',bg:'#FFFBEB',border:'#FDE68A'},{color:'#15803D',bg:'#F0FDF4',border:'#BBF7D0'},{color:'#B91C1C',bg:'#FEF2F2',border:'#FECACA'},{color:'#0891B2',bg:'#ECFEFF',border:'#A5F3FC'},{color:'#9D174D',bg:'#FDF2F8',border:'#FBCFE8'}]
  ...
  return <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.45)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999}} onClick={e=>e.target===e.currentTarget&&onClose()}><div style={{background:'#fff',borderRadius:16,width:420,maxWidth:'90vw',boxShadow:'0 20px 60px rgba(15,23,42,0.18)',overflow:'hidden'}}><div style={{padding:'18px 22px 14px',borderBottom:'1px solid #F1F5F9',display:'flex',alignItems:'center',justifyContent:'space-between'}}><div><div style={{fontSize:15,fontWeight:800,color:'#0F172A'}}>Phase anpassen</div><div style={{fontSize:11,color:'#94A3B8',marginTop:2}}>Nur fuer dich</div></div><button onClick={onClose} style={{background:'none',border:'none',cursor:'pointer',color:'#94A3B8',padding:4,borderRadius:6}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div><div style={{padding:'20px 22px',display:'flex',flexDirection:'column',gap:16}}><div><label style={{display:'block',fontSize:11,fontWeight:700,color:'#64748B',textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:6}}>Name</label><input value={lbl} onChange={e=>setLbl(e.target.value)} style={inp} placeholder="Phase benennen..." maxLength={20}/></div><div><label style={{display:'block',fontSize:11,fontWeight:700,color:'#64748B',textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:6}}>Beschreibung</label><input value={dsc} onChange={e=>setDsc(e.target.value)} style={inp} placeholder="Kurze Beschreibung..." maxLength={50}/></div><div><label style={{display:'block',fontSize:11,fontWeight:700,color:'#64748B',textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:6}}>Emoji</label><div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}><input value={ico} onChange={e=>setIco(e.target.value)} style={{...inp,width:64,fontSize:20,textAlign:'center'}} maxLength={4}/>{ICOS.map(em=><button key={em} onClick={()=>setIco(em)} style={{background:ico===em?'#EFF6FF':'#F8FAFC',border:'1px solid '+(ico===em?'#BFDBFE':'#E2E8F0'),borderRadius:8,padding:'4px 8px',cursor:'pointer',fontSize:18}}>{em}</button>)}</div></div><div><label style={{display:'block',fontSize:11,fontWeight:700,color:'#64748B',textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:6}}>Farbe</label><div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{C.map(o=><button key={o.color} onClick={()=>setClr(o)} style={{width:30,height:30,borderRadius:8,background:o.bg,border:'2px solid '+(clr.color===o.color?o.color:o.border),cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{width:14,height:14,borderRadius:4,background:o.color}}/></button>)}</div></div><div style={{background:'#F8FAFC',borderRadius:10,padding:'10px 14px',display:'flex',alignItems:'center',gap:10,border:'1px solid #E2E8F0'}}><span style={{fontSize:20}}>{ico}</span><div><div style={{fontWeight:800,fontSize:13,color:clr.color}}>{lbl||'Name'}</div><div style={{fontSize:10,color:'#94A3B8'}}>{dsc||'Beschreibung'}</div></div><span style={{marginLeft:'auto',fontSize:11,fontWeight:700,padding:'2px 10px',borderRadius:999,background:clr.bg,color:clr.color,border:'1px solid '+clr.border}}>0</span></div></div><div style={{padding:'14px 22px 18px',borderTop:'1px solid #F1F5F9',display:'flex',gap:10,justifyContent:'space-between',alignItems:'center'}}><button onClick={onReset} style={{fontSize:11,color:'#94A3B8',background:'none',border:'none',cursor:'pointer',textDecoration:'underline'}}>Alle zuruecksetzen</button><div style={{display:'flex',gap:8}}><button onClick={onClose} style={{padding:'9px 18px',borderRadius:8,border:'1px solid #E2E8F0',background:'#fff',color:'#475569',fontSize:13,fontWeight:600,cursor:'pointer'}}>Abbrechen</button><button onClick={()=>onSave({id:col.id,label:lbl.trim()||col.label,desc:dsc.trim(),icon:ico||col.icon,...clr})} style={{padding:'9px 18px',borderRadius:8,border:'none',background:'linear-gradient(135deg,#0A66C2,#1D4ED8)',color:'#fff',fontSize:13,fontWeight:700,cursor:'pointer'}}>Speichern</button></div></div></div></div>
}
