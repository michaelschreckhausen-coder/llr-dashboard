...
function DealCard({ lead, stage, onOpen, onMove, dragging, onDragStart, onDragEnd, stageConfig }) {
  ...
  const missingVal = !lead.deal_value && lead.deal_stage && !['kein_deal','verloren','gewonnen'].includes(lead.deal_stage)
...
export default function Pipeline({ session }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height: isMobile ? undefined : '100%', minHeight:0, flex:1 }}>
      {flash && (
                            ...
                            <div>
                              <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                                ...
                                <button onClick={e => { e.stopPropagation(); navigate(`/leads/${lead.id}`) }}
                                  style={{ padding:'2px 8px', borderRadius:6, border:'1px solid rgba(49,90,231,0.25)', background:'rgba(49,90,231,0.07)', color:'var(--wl-primary, rgb(49,90,231))', fontSize:10, fontWeight:700, cursor:'pointer' }}>↗ Profil</button>
                              </div>
