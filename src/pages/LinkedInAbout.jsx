...
export default function LinkedInAbout({ session, sub }) {
  ...
  var labelStyle = { fontSize: 11, fontWeight: 700, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 7, display: 'block' }

  function makeOptBtn(active, onClick, main, sub) {
    return React.createElement('button', {
      onClick: onClick,
      style: {
        padding: '9px 12px', borderRadius: 9, cursor: 'pointer', textAlign: 'left', width: '100%', display: 'block',
        border: '1.5px solid ' + (active ? '#0A66C2' : '#E2E8F0'),
        background: active ? '#EFF6FF' : '#F8FAFC', transition: 'all 0.15s', marginBottom: 6
      }
    },
  ...
  return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 1100 } },
    ...
    React.createElement('div', {
      ...
      React.createElement('div', null,
        React.createElement('div', { style: { fontSize: 20, fontWeight: 800, color: '#fff', letterSpacing: '-0.02em', marginBottom: 4 } }, 'LinkedIn Info-Bereich schreiben'),
        React.createElement('div', { style: { fontSize: 13, color: 'rgba(255,255,255,0.8)' } }, 'KI generiert deinen About-Text aus Profil und Brand Voice')
      ),
      profile && profile.full_name ? React.createElement('div', {
        style: { background: 'rgba(255,255,255,0.15)', borderRadius: 10, padding: '10px 16px', border: '1px solid rgba(255,255,255,0.2)', flexShrink: 0 }
      },
    ...
    React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 } },
      ...
      React.createElement('div', null,
          ...
          React.createElement('div', { style: cardBodyStyle },
            ...
            React.createElement('div', {
              ...
              React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } },
                React.createElement('div', { style: { width: 8, height: 8, borderRadius: '50%', background: (profile && profile.full_name) ? '#10B981' : '#F59E0B' } }),
                React.createElement('div', null,
                  ...
                  React.createElement('div', { style: { fontSize: 11, color: '#64748B' } },
                    (profile && profile.full_name) ? (profile.headline || profile.company || 'Profil hinterlegt') : 'Profil unvollstaendig'
                  )
                )
              ),
              React.createElement('a', { href: '/profile', style: { fontSize: 11, fontWeight: 700, color: '#0A66C2', textDecoration: 'none', background: '#EFF6FF', padding: '3px 10px', borderRadius: 999, border: '1px solid #BFDBFE' } }, 'Bearbeiten')
            ),

            /* Brand Voice */
            React.createElement('label', { style: labelStyle }, 'Brand Voice'),
            brandVoices.length === 0
              ? React.createElement('div', { style: { padding: '9px 12px', borderRadius: 9, background: '#FFF7ED', border: '1px solid #FDE68A', fontSize: 12, color: '#92400E', display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
              ...
              : React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 6 } },
                  React.createElement('label', { style: { display: 'flex', alignItems: 'center', gap: 9, padding: '8px 11px', borderRadius: 9, cursor: 'pointer', border: '1.5px solid ' + (selectedBrand === 'auto' ? '#0A66C2' : '#E2E8F0'), background: selectedBrand === 'auto' ? '#EFF6FF' : '#F8FAFC' } },
                    React.createElement('input', { type: 'radio', name: 'brand', value: 'auto', checked: selectedBrand === 'auto', onChange: function() { setSelectedBrand('auto') }, style: { accentColor: '#0A66C2' } }),
                    React.createElement('div', null,
                      React.createElement('div', { style: { fontSize: 12, fontWeight: 700, color: selectedBrand === 'auto' ? '#0A66C2' : '#0F172A' } }, 'Automatisch (aktive Voice)'),
                      activeBrand ? React.createElement('div', { style: { fontSize: 11, color: '#64748B' } }, activeBrand.name) : null
                    )
                  ),
                  brandVoices.map(function(bv) {
                    return React.createElement('label', { key: bv.id, style: { display: 'flex', alignItems: 'center', gap: 9, padding: '8px 11px', borderRadius: 9, cursor: 'pointer', border: '1.5px solid ' + (selectedBrand === bv.id ? '#0A66C2' : '#E2E8F0'), background: selectedBrand === bv.id ? '#EFF6FF' : '#F8FAFC' } },
                  ...
                  React.createElement('label', { style: { display: 'flex', alignItems: 'center', gap: 9, padding: '8px 11px', borderRadius: 9, cursor: 'pointer', border: '1.5px solid ' + (selectedBrand === 'none' ? '#0A66C2' : '#E2E8F0'), background: selectedBrand === 'none' ? '#EFF6FF' : '#F8FAFC' } },
                    React.createElement('input', { type: 'radio', name: 'brand', value: 'none', checked: selectedBrand === 'none', onChange: function() { setSelectedBrand('none') }, style: { accentColor: '#0A66C2' } }),
                    React.createElement('div', { style: { fontSize: 12, fontWeight: 600, color: '#64748B' } }, 'Ohne Brand Voice')
                  )
                )
          )
        ),
        ...
        React.createElement('div', { style: cardStyle },
          ...
          React.createElement('div', { style: cardBodyStyle },
            ...
            React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7, marginBottom: 14 } },
              VARIANTS.map(function(v) { return React.createElement('div', { key: v.id }, makeOptBtn(variant === v.id, function() { setVariant(v.id) }, v.label, v.desc)) })
            ),
            ...
            React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 7, marginBottom: 14 } },
              LENGTHS.map(function(l) { return React.createElement('div', { key: l.id }, makeOptBtn(length === l.id, function() { setLength(l.id) }, l.label, l.desc)) })
            ),
        ...
        React.createElement('div', { style: cardStyle },
          React.createElement('div', { style: cardHeaderStyle },
            React.createElement('span', { style: { fontSize: 13, fontWeight: 700, color: '#0F172A' } }, 'Schwerpunkte (' + focusAreas.length + ' gewaehlt)')
          ),
          React.createElement('div', { style: cardBodyStyle },
            React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 7 } },
              FOCUS_AREAS.map(function(area) {
                ...
                return React.createElement('button', {
                  key: area,
                  onClick: function() { toggleFocus(area) },
                  style: { padding: '4px 12px', borderRadius: 999, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none', background: isActive ? '#0A66C2' : '#F1F5F9', color: isActive ? '#fff' : '#475569', transition: 'all 0.15s' }
                }, area)
              })
            )
          )
        ...
        React.createElement('div', { style: cardStyle },
          React.createElement('div', { style: cardHeaderStyle },
            React.createElement('span', { style: { fontSize: 13, fontWeight: 700, color: '#0F172A' } }, 'Zusaetzliche Infos (optional)')
          ),
          React.createElement('div', { style: cardBodyStyle },
            React.createElement('textarea', {
              value: extraInfo,
              onChange: function(e) { setExtraInfo(e.target.value) },
              rows: 4,
              placeholder: 'Besondere Erfolge, Keywords, aktuelle Projekte...',
              style: { width: '100%', padding: '9px 12px', border: '1.5px solid #E2E8F0', borderRadius: 9, fontSize: 13, fontFamily: 'inherit', resize: 'vertical', outline: 'none', lineHeight: 1.6, boxSizing: 'border-box' }
            })
          )
        ),
        ...
        !(profile && profile.full_name) ? React.createElement('div', {
          style: { marginTop: 10, padding: '9px 14px', background: '#FFF7ED', border: '1px solid #FDE68A', borderRadius: 9, fontSize: 12, color: '#92400E', fontWeight: 600 }
        }, 'Bitte zuerst Profil ausfuellen') : null,

        error ? React.createElement('div', {
          style: { marginTop: 10, padding: '9px 14px', background: '#FEF2F2', border: '1px solid #FCA5A5', borderRadius: 9, fontSize: 12, color: '#991B1B', fontWeight: 600 }
        }, error) : null
      ),
      ...
      React.createElement('div', null,
        ...
        React.createElement('div', { style: Object.assign({}, cardStyle, { marginBottom: 14 }) },
          React.createElement('div', { style: Object.assign({}, cardHeaderStyle, { display: 'flex', alignItems: 'center', justifyContent: 'space-between' }) },
            ...
            result ? React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10 } },
              React.createElement('span', { style: { fontSize: 12, fontWeight: 700, color: charOver ? '#EF4444' : charWarn ? '#F59E0B' : '#10B981' } }, charCount + ' / ' + charMax),
              React.createElement('button', {
                onClick: generate, disabled: generating,
                style: { padding: '5px 12px', borderRadius: 8, border: '1px solid #E2E8F0', background: 'transparent', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: '#64748B' }
              }, 'Neu'),
              React.createElement('button', {
                onClick: copyText,
                style: { padding: '5px 14px', borderRadius: 8, border: 'none', background: copied ? '#DCFCE7' : '#0A66C2', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: copied ? '#065F46' : '#fff' }
          ...
          result ? React.createElement('div', { style: { padding: 18 } },
            /* LinkedIn preview */
            React.createElement('div', { style: { background: '#F3F2EF', borderRadius: 10, padding: 14, border: '1px solid #E2E8F0', marginBottom: 14 } },
              React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 9, marginBottom: 10 } },
                React.createElement('div', { style: { width: 38, height: 38, borderRadius: '50%', background: 'linear-gradient(135deg,#0A66C2,#3B82F6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 800, color: '#fff', flexShrink: 0 } },
                  (profile && profile.full_name) ? profile.full_name.charAt(0).toUpperCase() : '?'
                ),
                React.createElement('div', null,
                  React.createElement('div', { style: { fontSize: 13, fontWeight: 700, color: '#0F172A' } }, profile && profile.full_name),
                  profile && profile.headline ? React.createElement('div', { style: { fontSize: 11, color: '#64748B' } }, profile.headline) : null
                )
              ),
              React.createElement('div', { style: { fontSize: 11, fontWeight: 700, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 7 } }, 'Info'),
              React.createElement('div', { style: { fontSize: 13, color: '#0F172A', lineHeight: 1.7, whiteSpace: 'pre-wrap' } }, result)
            ),
            charOver ? React.createElement('div', { style: { marginBottom: 10, padding: '8px 12px', background: '#FEF2F2', border: '1px solid #FCA5A5', borderRadius: 8, fontSize: 12, color: '#991B1B', fontWeight: 600 } },
              'Text ist ' + (charCount - charMax) + ' Zeichen zu lang.'
            ) : null,
            ...
            React.createElement('textarea', {
              ...
              rows: 10,
              style: { width: '100%', padding: '11px 13px', border: '1.5px solid #E2E8F0', borderRadius: 9, fontSize: 13, fontFamily: 'inherit', resize: 'vertical', outline: 'none', lineHeight: 1.7, boxSizing: 'border-box', color: '#0F172A' }
            }),
            React.createElement('button', {
              onClick: copyText,
        ...
        /* Tips */
        React.createElement('div', { style: { background: 'linear-gradient(135deg,#F0F7FF,#EFF6FF)', borderRadius: 12, border: '1px solid #BFDBFE', padding: '14px 18px' } },
          React.createElement('div', { style: { fontSize: 12, fontWeight: 700, color: '#1D4ED8', marginBottom: 9 } }, 'Tipps fuer deinen LinkedIn Info-Bereich'),
          React.createElement('ul', { style: { margin: 0, paddingLeft: 16, display: 'flex', flexDirection: 'column', gap: 5 } },
            [
              ...
              'Konkreter CTA am Ende erhoeht Kontaktanfragen',
