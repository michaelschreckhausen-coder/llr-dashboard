...
function KPICard({ icon, label, value, sub, color = '#0A66C2', trend }) {
  return (
    <div style={{
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ width: 40, height: 40, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, background: color + '15', flexShrink: 0 }}>{icon}</div>
        {trend !== undefined && (
          <span style={{ fontSize: 11, fontWeight: 700, color: trend >= 0 ? '#10B981' : '#EF4444', background: trend >= 0 ? '#ECFDF5' : '#FEF2F2', padding: '2px 7px', borderRadius: 999, border: '1px solid ' + (trend >= 0 ? '#A7F3D0' : '#FCA5A5') }}>
            {trend >= 0 ? '+' : ''}{trend}%
          </span>
        )}
      </div>
...
function TaskRow({ task, onIncrement }) {
  ...
  const taskIcons = { comment: '\ud83d\udcac', lead: '\ud83d\udc64', profile: '\ud83d\udd0d', message: '\u2709\ufe0f', post: '\u270f\ufe0f' }
  return (
    <div style={{ padding: '12px 14px', borderRadius: 10, background: task.completed ? '#F0FDF4' : '#F8FAFC', border: '1px solid ' + (task.completed ? '#A7F3D0' : '#E2E8F0'), display: 'flex', alignItems: 'center', gap: 12, transition: 'all 0.15s' }}>
      <div style={{ width: 36, height: 36, borderRadius: 9, flexShrink: 0, background: task.completed ? '#DCFCE7' : color + '15', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>
        {task.completed ? '\u2713' : taskIcons[task.type]}
      </div>
      ...
      {!task.completed && (
        <button onClick={() => onIncrement(task.type)} style={{ background: color, color: '#fff', border: 'none', borderRadius: 8, padding: '5px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer', flexShrink: 0, transition: 'all 0.15s' }}
...
export default function Dashboard({ session }) {
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300, color: '#94A3B8', gap: 12, fontSize: 14 }}>
      \u23f3 Lade Dashboard\u2026
    </div>
  )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.75)', fontWeight: 400 }}>Mehr Leads. Mehr Sichtbarkeit. Mehr Umsatz.</div>
        </div>
        <div style={{ background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(8px)', borderRadius: 12, padding: '14px 22px', textAlign: 'center', border: '1px solid rgba(255,255,255,0.2)', flexShrink: 0 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14 }}>
        ...
        <KPICard icon="\u26a1"         label="Engagement Score"  value={score}                         color="#8B5CF6" sub="Komm\xd72 + Leads\xd73"/>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: 20 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', boxShadow: '0 1px 3px rgba(15,23,42,0.06)', overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#0F172A' }}>\ud83d\udccb Tägliche Aufgaben</div>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', boxShadow: '0 1px 3px rgba(15,23,42,0.06)' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#0F172A' }}>\ud83d\udcca Wöchentliche Aktivität</div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: '#64748B' }}>Kommentare \xd7 2 + Leads \xd7 3</div>
            </div>
          </div>
