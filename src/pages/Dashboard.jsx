    ...
    <div style={{ padding:'12px 14px', borderRadius:10, background:task.completed?'#F0FDF4':'#F8FAFC', border:'1px solid '+(task.completed?'#A7F3D0':'#E2E8F0'), display:'flex', alignItems:'center', gap:12, transition:'all 0.15s' }}>
      ...
      {!task.completed && (
        <button onClick={() => onIncrement(task.type)}
          style={{ background:color, color:'#fff', border:'none', borderRadius:8, padding:'5px 12px', fontSize:12, fontWeight:700, cursor:'pointer', flexShrink:0 }}
...
function HotLeadRow({ lead }) {
  ...
  return (
    <div
      style={{ padding:'10px 16px', borderBottom:'1px solid #F8FAFC', display:'flex', alignItems:'center', gap:10, background:hovered?'#F8FAFC':'#fff', transition:'background 0.15s' }}
    ...
    >
      ...
      <div style={{ display:'flex', gap:6, opacity:hovered?1:0, transition:'opacity 0.15s' }}>
        {lead.linkedin_url && (
          <a href={lead.linkedin_url} target="_blank" rel="noreferrer" style={{ fontSize:11, color:'#0A66C2', fontWeight:700, textDecoration:'none', padding:'4px 10px', borderRadius:6, border:'1px solid #BFDBFE', background:'#EFF6FF', whiteSpace:'nowrap' }}>LinkedIn →</a>
        )}
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:24 }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0A66C2 0%,#1D4ED8 60%,#3B82F6 100%)', borderRadius:16, padding:'28px 32px', display:'flex', alignItems:'center', justifyContent:'space-between', boxShadow:'0 4px 20px rgba(10,102,194,0.28)', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ zIndex:1 }}>
          ...
          <div style={{ fontSize:14, color:'rgba(255,255,255,0.85)', fontWeight:400 }}>
            {leadsTotal === 0 ? 'Leg los — importiere deinen ersten Lead!' : 'Mehr Leads. Mehr Sichtbarkeit. Mehr Umsatz.'}
          </div>
          {leadsTotal === 0 && (
          ...
          <div style={{ padding:'14px 16px', display:'flex', flexDirection:'column', gap:8 }}>
            {tasks.length === 0 ? (
              <div style={{ padding:'24px 0', textAlign:'center', color:'#94A3B8', fontSize:13 }}>
                ...
                <div style={{ fontSize:12, marginTop:4 }}>Tasks werden automatisch erstellt</div>
              </div>
