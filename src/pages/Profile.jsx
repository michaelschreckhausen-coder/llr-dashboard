...
export default function Profile({ session }) {
  ...
  async function load() {
    ...
    const li = user?.identities?.find(i => i.provider === 'linkedin_oidc') || null
    setLiIdentity(li)
    const liName    = li?.identity_data?.name || li?.identity_data?.full_name || ''
    const liPicture = li?.identity_data?.avatar_url || li?.identity_data?.picture || ''
    const merged = {
    ...
    setForm({ full_name: merged.full_name, company: merged.company, headline: merged.headline, bio: merged.bio })
  }
  ...
  async function syncFromLinkedIn() {
    ...
    const li = liIdentity.identity_data
    const liPicture = li?.avatar_url || li?.picture || ''
    const newForm = {
      full_name: li?.name || li?.full_name || form.full_name,
      company:   form.company,
      headline:  li?.job_title || li?.headline || form.headline,
      bio:       form.bio,
    }
    setForm(newForm)
    const update = { ...newForm, ...(liPicture ? { avatar_url: liPicture } : {}) }
    await supabase.from('profiles').update(update).eq('id', session.user.id)
    setProfile(p => ({ ...p, ...update }))
    setMsg({ type:'success', text: 'Profildaten von LinkedIn uebernommen!' })
  ...
  const avatarUrl   = profile?.avatar_url || liIdentity?.identity_data?.avatar_url || liIdentity?.identity_data?.picture || ''
  const displayName = profile?.full_name  || session.user.email?.split('@')[0] || 'Unbekannt'
  const initials    = (profile?.full_name || session.user.email || '?').split(' ').map(w => w[0]).join('').substring(0, 2).toUpperCase()
  ...
  const S = {
    ...
    msgBox: (t) => ({ padding:'10px 14px', borderRadius:8, fontSize:13, background:t==='success'?'#e6f4ee':'#fde8e8', color:t==='success'?'#057642':'#cc1016', border:'1px solid ' + (t==='success'?'#b7dfc9':'#f5b8b8') }),
  }
  ...
  return (
    <div style={S.page}>
      <div style={{ marginBottom:24 }}>
        ...
        <div style={{ color:'#888', fontSize:14 }}>Verwalte deine persoenlichen Informationen</div>
      </div>
        ...
        <div style={{ padding:'0 22px 22px', position:'relative' }}>
          ...
          {liIdentity && (
            <div style={{ position:'absolute', top:16, right:22, display:'flex', alignItems:'center', gap:7, padding:'6px 12px', borderRadius:16, background:'#e8f0fb', border:'1px solid #bfdbfe' }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color:LI_BLUE }}>LinkedIn verknuepft</span>
            </div>
          )}
        </div>
      </div>
      ...
      {liIdentity && (
        <div style={{ background:'#f0f7ff', border:'1px solid #bfdbfe', borderRadius:10, padding:'14px 18px', marginBottom:16, display:'flex', alignItems:'center', justifyContent:'space-between', gap:16 }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div>
              <div style={{ fontWeight:700, fontSize:13, color:LI_BLUE }}>LinkedIn-Daten uebernehmen</div>
              <div style={{ fontSize:12, color:'#555', marginTop:1 }}>Name und Profilbild automatisch von LinkedIn importieren</div>
            </div>
          </div>
          <button onClick={syncFromLinkedIn}
            style={{ display:'flex', alignItems:'center', gap:7, padding:'8px 16px', borderRadius:16, border:'none', background:LI_BLUE, color:'white', fontSize:12, fontWeight:700, cursor:'pointer', whiteSpace:'nowrap', flexShrink:0 }}
      ...
      <div style={S.card}>
        ...
        <div style={S.body}>
          ...
          <div style={S.row2}>
            <div>
              <label style={S.lbl}>Vollstaendiger Name</label>
          ...
          <div>
            <label style={S.lbl}>Ueber mich</label>
            <textarea value={form.bio} onChange={e => setForm(f => ({...f, bio:e.target.value}))} placeholder="Kurze Beschreibung ueber dich und deine Arbeit..." rows={4}
      ...
      <div style={S.card}>
        ...
        <div style={{ padding:'18px 22px', display:'flex', flexDirection:'column', gap:10 }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 14px', background:liIdentity?'#e8f5ee':'#fafafa', borderRadius:8, border:'1px solid ' + (liIdentity?'#86efac':BORDER) }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <LinkedInIcon size={16} color={liIdentity?LI_BLUE:'#ccc'}/>
              <div>
                <div style={{ fontSize:11, color:'#aaa', marginBottom:2, textTransform:'uppercase', letterSpacing:'0.5px', fontWeight:700 }}>LinkedIn</div>
                <div style={{ fontSize:14, fontWeight:600, color:liIdentity?'#057642':'#888' }}>
                  {liIdentity ? 'Verknuepft - ' + (liIdentity.identity_data?.email || 'Account verbunden') : 'Nicht verknuepft'}
                </div>
              </div>
            </div>
            {!liIdentity && <a href="/settings" style={{ fontSize:12, color:LI_BLUE, fontWeight:600, textDecoration:'none' }}>Verknuepfen</a>}
          </div>
        </div>
      </div>
    </div>
